...
describe("conteract", () => {
  it("should deploy correctly", async () => {
    ...
    const master = await system.treasury('master');
    const nft = system.openContract(
    ...
    const nft_tracker = system.track(nft.address);

    await nft.send(master, 
      {value: toNano("0.1")},
      {$$type: 'Deploy', query_id: 0n}
    );
...
//          const contract = system.openContract(
//            await SampleTactContract.fromInit(owner.address, 0n)
//          );
//         const deployResult = await contract.send(
//            owner.getSender(), { value: toNano(1) }, null);
...
//         // Increment counter
//         await contract.send(
//            owner.getSender(), { value: toNano(1) }, { $$type: "Increment" }
//          );
